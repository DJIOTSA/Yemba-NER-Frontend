...
async function getHistoryData() {
    ...
    if (response.ok) {
        ...
        if (elements.length >= 1) {
            ...
            history.querySelector('.table').style.display = "block";
            history.querySelector('.table').innerHTML = `<p class="text-ner text-center text-lead"> <i class="fa fa-info px-1" aria-hidden="true"></i>You don't have stored history yet!</p>`
        }
    } else {
        historyLoader.style.display ='none';
